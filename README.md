...
**Analysis:** HTAC achieves the highest average performance (0.71) across all 4 task orderings with minimal variance (std across orders < 0.02). Structural baselines (PackNet=0.65, Grow=0.52) show more order-sensitive variance, particularly Grow (Order2=0.43 vs Order1=0.54). Regularization methods all remain below 0.30. The consistent performance of HTAC confirms that the hierarchical domain/task expert structure and on-demand creation mechanism are robust to task arrival order.

---

## Table S2: FWT Decomposition on OCW-20 (HTAC, 3 Seeds)

**FWT Formula:**
$$\text{FWT} = \frac{1}{K-1}\sum_{i=2}^{K}\bigl(p_{\text{matrix}}[i][i+1] - p_{\text{init}}[i+1]\bigr)$$

where $p_{\text{init}} \approx 0$ (random policy), so FWT equals the zero-shot success rate on task $i+1$ after training task $i$.
...
**Detailed Analysis:**
- **Tasks 1→10 (0.018):** OCW-10 spans fundamentally distinct operations (push, grasp, rotate, peg), making large-scale task generalization inherently difficult. The positive value (vs. 0 for random baseline) indicates domain experts do encode transferable coarse-grained knowledge.
- **Tasks 11→20 (0.700):** When the system encounters semantically identical tasks in round 2, the domain prototype identifies the match (S-BERT similarity > τ), the warmup evaluation confirms viability (success rate > θ), and the frozen round-1 expert is directly deployed—no fine-tuning required. This achieves near-perfect knowledge reactivation.
...
## Table S3: Threshold θ Sensitivity Analysis (OCW-20, 3 Seeds)
...
## Table S4: Domain Number D Ablation (OCW-10, 3 Seeds)
...
**Analysis:**
- **D too small (1–3):** Intra-domain task diversity becomes too large. Domain experts cannot focus on consistent shared structure, leading to increased forgetting (D=3: F=0.03) and reduced performance.
...
- Note: For other benchmarks with different task structures, D should be tuned accordingly; we recommend setting D to match the number of distinct manipulation primitive categories.

---

## Table S5: HTAC Parameter Breakdown (CW-10)
